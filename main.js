...
while (true) {
    ...
    marsLander.vSpeed = parseInt(inputs[3]); // the vertical speed (in m/s), can be negative.
    ...
    printErr("vSpeed" + marsLander.vSpeed);
    if (marsLander.vSpeed > -40) {
        marsLander.power = 0;
    }
    else {
        marsLander.power = 4;
    ...
    // if marslander.hspeed >50ms , changer la power, peut etre avec une methode

    // 2 integers: rotate power. rotate is the desired rotation angle (should be 0 for level 1), power is the desired thrust power (0 to 4).
    print('0 ' + marsLander.power);
}
